...
public class RealChunk {
    ...
    public static RealChunk readChunk(DataSource dataSource)
            throws CannotReadException, IOException {
        final String id = Utils.readString(dataSource, 4);
        final int size = (int)Utils.readUint32(dataSource);
        if (size < 8) {
            throw new CannotReadException(
                    "Corrupt file: RealAudio chunk length at position "
                            + (dataSource.position() - 4)
                            + " cannot be less than 8");
        }
        if (size > (dataSource.size() - dataSource.position() + 8)) {
            throw new CannotReadException(
                    "Corrupt file: RealAudio chunk length of " + size
                            + " at position " + (dataSource.position() - 4)
